...
from typing import Optional, Tuple, Dict, Any, List, Sequence, Mapping, Iterable, Union
...
class CoverHero(Flowable):
    ...
    def draw(self) -> None:  # pragma: no cover - solo dibujo
        canv = self.canv
        steps = 48
        start = _mix_colors(self.accent, colors.white, 0.7)
        end = colors.white
        ...
        overlay.close()
        canv.setFillColor(_mix_colors(self.accent, colors.white, 0.9))
...
def _pdf_card(
    ...
    padding: Tuple[float, float, float, float] = (12, 12, 18, 18),
) -> Table:
    ...
    highlight = _lighten_color(accent, 0.85)
    blended_background = _mix_colors(base_background, highlight, 0.4)
    border_color = _mix_colors(accent, colors.white, 0.3)
...
class MainApp:
    ...
    def __init__(self, page: ft.Page):
        ...
        try:
            stored_time_unit = self.page.client_storage.get("time_display_unit")
        except Exception:
            stored_time_unit = None
        valid_time_units = {"vel_mm", "acc", "acc_g"}
        self.time_display_unit = stored_time_unit if stored_time_unit in valid_time_units else "vel_mm"
        ...
        self.pdf_progress_overlay = self._build_loading_overlay(
            key="pdf_progress",
            icon=ft.Icons.PICTURE_AS_PDF,
            message_control=self.pdf_progress_text,
            hint_message="Estamos preparando tu reporte.",
        )
        ...
        self.file_loading_overlay = self._build_loading_overlay(
            key="file_loading",
            icon=ft.Icons.UPLOAD_FILE,
            message_control=self.file_loading_text,
            hint_message="Validamos el contenido, esto puede tardar unos segundos.",
        )
        ...
        try:
            ...
            self.page.theme = ft.Theme(
                ...
                icon_button_theme=ft.IconButtonTheme(
                    foreground_color=accent,
                    hover_color=Colors.with_opacity(0.12, accent),
                ),
                button_theme=ft.ButtonTheme(
                    button_color=accent,
                    splash_color=Colors.with_opacity(0.2, accent),
                ),
            )
        except Exception:
            pass
    ...
    def _remember_color_pref(self, key: str, selected: Optional[str], fallback: str) -> str:
        """Normaliza y persiste el color elegido para las gráficas."""

        resolved = selected if selected not in (None, "", "None") else fallback
        sanitized = self._sanitize_color(resolved, fallback)
        try:
            self.page.client_storage.set(key, sanitized)
        except Exception:
            pass
        return sanitized
    ...
    def _count_active_axes(self, axis_entries: Optional[Iterable[Mapping[str, Any]]]) -> int:
        """Cuenta los ejes no globales disponibles en los resúmenes."""

        try:
            return sum(
                1
                for entry in (axis_entries or [])
                if isinstance(entry, Mapping) and not entry.get("is_global")
            )
        except Exception:
            return 0

    def _iso_scope_label(self, axis_count: int, scope: str = "global") -> str:
        """Devuelve una etiqueta compacta para describir el alcance ISO mostrado."""

        try:
            count = int(axis_count)
        except Exception:
            count = 0
        scope_txt = scope or "global"
        if count <= 0:
            return f"ISO {scope_txt}"
        plural = "eje" if count == 1 else "ejes"
        return f"ISO {scope_txt} ({count} {plural})"

    def _iso_scope_note(
        self,
        axis_count: int,
        primary_entry: Optional[Mapping[str, Any]],
        primary_rms: Optional[float],
    ) -> Optional[str]:
        """Aclara la diferencia entre el semáforo ISO y el eje analizado."""
        ...
        try:
            ...
            count = 0
        if count <= 0:
            return "El semáforo ISO representa el RMS global calculado con las señales disponibles."
        plural = "eje" if count == 1 else "ejes"
        if count >= 1:
            return (
                f"El semáforo ISO refleja el RMS global combinado de {count} {plural}; la condición reportada "
                "corresponde al conjunto completo de ejes analizados."
            )

    def _build_severity_traffic_light(
        self,
        rms_mm: float,
        *,
        ...
        def _label_index(label: Optional[str]) -> int:
            ...
            if "satisf" in text:
                return 1
            if "buena" in text or "aceptable" in text:
                return 0
            return -1
        ...
        active_idx = _label_index(iso_label)
        if active_idx < 0:
            if rms_val <= 2.8:
                active_idx = 0
            elif rms_val <= 4.5:
        ...
        palette = [
            ("#2ecc71", "Zona A (Buena)"),
            ("#f1c40f", "Zona B (Vigilancia)"),
            ("#e67e22", "Zona C (Insatisfactoria)"),
            ("#e74c3c", "Zona D (Inaceptable)"),
        ]
        lights: List[ft.Control] = []
        for idx, (color, label) in enumerate(palette):
            ...
            lights.append(
                ft.Column(
                    [
                        ...
                        ft.Text(label, size=11, text_align=ft.TextAlign.CENTER, width=110),
                    ],
                    spacing=4,
        ...
        header = "Semáforo ISO"
        if scope_text:
            header = f"{header} – {scope_text}"
        current_label = iso_label or self._classify_severity(rms_val)

        return ft.Container(
            content=ft.Column(
                [
                    ft.Text(header, weight="bold", size=13),
                    ft.Row(lights, alignment="spaceAround", expand=True),
                    ft.Text(
                        f"Nivel reportado: {current_label}",
                        size=11,
                        color="#7f8c8d",
    ...
    def _axis_order_index(self, column: Any) -> int:
        letter = self._axis_letter_from_name(column)
        order_map = {"X": 0, "Y": 1, "Z": 2}
        return order_map.get(letter or "", 99)

    def _sort_axis_entries(self, entries: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
        normalized: List[Dict[str, Any]] = [entry for entry in entries if isinstance(entry, dict)]
        return sorted(
            normalized,
            key=lambda entry: (
                self._axis_order_index(entry.get("column") or entry.get("name")),
                str(entry.get("name", "")),
            ),
        )

    def _compose_axis_narrative(
        self,
        entries: Iterable[Dict[str, Any]],
        global_label: Optional[str] = None,
        global_rms: Optional[float] = None,
    ) -> str:
        axis_entries = self._sort_axis_entries(entry for entry in entries if entry and not entry.get("is_global"))
        fragments: List[str] = []
        for entry in axis_entries[:3]:
            try:
                rms_val = float(entry.get("rms_mm_s", 0.0))
                rms_txt = f"{rms_val:.3f} mm/s"
            except Exception:
                rms_txt = "N/D"
            iso_label = str(entry.get("iso_label", "N/D"))
            ml_summary = entry.get("ml_summary") or {}
            ml_status = str(ml_summary.get("status") or "").lower()
            if ml_status == "ok":
                ml_txt = ml_summary.get("final_label") or ml_summary.get("ml_label")
                ml_clause = f", ML {ml_txt}" if ml_txt else ""
            elif ml_status:
                ml_clause = ", ML sin disponibilidad"
            else:
                ml_clause = ""
            fragments.append(f"{entry.get('name', 'Eje')}: {rms_txt} ({iso_label}{ml_clause})")
        if global_label and global_rms is not None:
            fragments.append(f"RMS global {global_rms:.3f} mm/s ({global_label})")
        if not fragments:
            return "No se dispone de medidas por eje para esta medicion."
        if len(fragments) == 1:
            return fragments[0] + "."
        return "; ".join(fragments[:-1]) + " y " + fragments[-1] + "."

    def _compose_findings_paragraph(self, findings: Sequence[str]) -> str:
        cleaned = [str(item).strip().rstrip(".") for item in findings if str(item).strip()]
        if not cleaned:
            return "Sin hallazgos adicionales mas alla de la severidad global."
        if len(cleaned) == 1:
            return cleaned[0] + "."
    ...
    def _get_axis_columns(
        ...
        preferred_letters: Dict[str, str] = {}
        ...
        for col in data.columns:
            ...
            if unit == "acc_ms2":
                ...
                name = str(col).lower()
                candidate = ("acc" in name) or ("acel" in name) or bool(self._axis_letter_from_name(col))
            if not candidate:
                continue
            letter = self._axis_letter_from_name(col)
            if letter and letter not in preferred_letters:
                preferred_letters[letter] = col
            else:
                fallback_cols.append(col)
        axis_cols: List[str] = []
        for axis_letter in ("X", "Y", "Z"):
            col = preferred_letters.get(axis_letter)
            if col and col not in axis_cols:
                axis_cols.append(col)
        for col in fallback_cols:
            if col not in axis_cols:
                axis_cols.append(col)
            if len(axis_cols) >= 3:
                break
        if not axis_cols:
            axis_cols = fallback_cols[:3]
        return axis_cols[:3] if axis_cols else []
    ...
    def exportar_pdf(self, e=None):
        ...
        try:
            ...
            current_source = getattr(self, "current_file_path", None)
            if not current_source:
                files = getattr(self, "uploaded_files", None) or []
                if files:
                    current_source = files[-1]
            try:
                base_name = Path(current_source).stem if current_source else "sin_nombre"
            except Exception:
            ...
            axis_triplet_pdf = self._sort_axis_entries(
                entry for entry in (axis_summaries_pdf or []) if not entry.get("is_global")
            )
            axis_triplet_pdf = axis_triplet_pdf[:3]
            axis_count_pdf = self._count_active_axes(axis_summaries_pdf)
            iso_scope_label_pdf = self._iso_scope_label(axis_count_pdf, scope="global")
            iso_scope_note_pdf = self._iso_scope_note(axis_count_pdf, primary_entry_pdf, primary_rms_mm_pdf)
            if axis_triplet_pdf:
                axis_table_rows.append(["Canal", "RMS (mm/s)", "Clasificacion ISO", "Diagnostico ML"])
                for entry in axis_triplet_pdf:
                    try:
            ...
            try:
                if getattr(self, "trend_fft_snapshots", None):
                    trend_fig = self._generate_trend_figure(force_light=True)
            ...
            def _draw_header_footer(canvas, doc_obj):  # pragma: no cover - dibujo
                canvas.saveState()
                header_height = 32
                header_bg = _mix_colors(accent_color, colors.white, 0.85)
                primary_text = colors.HexColor("#1f2a37")
                secondary_text = colors.HexColor("#4b5563")
                canvas.setFillColor(header_bg)
                canvas.rect(0, A4[1] - header_height, A4[0], header_height, fill=1, stroke=0)
                left_margin = getattr(doc_obj, 'leftMargin', 50)
                right_margin = getattr(doc_obj, 'rightMargin', 50)
                top_text_y = A4[1] - header_height + 10
                footer_y = getattr(doc_obj, 'bottomMargin', 48) - 18
                right_x = A4[0] - right_margin
                canvas.setFillColor(primary_text)
                canvas.setFont('Helvetica-Bold', 11)
                canvas.drawString(left_margin, top_text_y, 'Informe de Vibraciones')
                canvas.setFont('Helvetica', 8)
                canvas.drawRightString(right_x, top_text_y, base_name)
                canvas.setFillColor(secondary_text)
                canvas.setFont('Helvetica', 8)
                canvas.drawString(left_margin, max(footer_y, 18), generated_at.strftime('%d/%m/%Y %H:%M'))
                canvas.drawRightString(right_x, max(footer_y, 18), f'Pagina {doc_obj.page}')
                canvas.restoreState()
            ...
            styles.add(
                ParagraphStyle(
                    "Muted",
                    parent=styles['Normal'],
                    textColor=colors.HexColor("#5f6a6a"),
                    fontSize=9,
                    leading=12,
                )
            )
            ...
            def _build_metric_tiles(metrics: List[Tuple[str, ...]]) -> Optional[Table]:
                if not metrics:
                    return None
                ...
                for entry in metrics:
                    ...
                    if isinstance(entry, Mapping):
                        title_txt = str(entry.get("title", ""))
                        subtitle_txt = str(entry.get("subtitle", ""))
                    elif isinstance(entry, (list, tuple)):
                        if len(entry) >= 3:
                            _, title_txt, subtitle_txt = entry[:3]
                        elif len(entry) == 2:
                            title_txt, subtitle_txt = entry
                        elif len(entry) == 1:
                            title_txt = entry[0]
                            subtitle_txt = ""
                        else:
                        ...
                        title_txt = str(entry)
                    normalized.append((str(title_txt), str(subtitle_txt), ""))
                if not normalized:
                    return None
                col_count = len(normalized)
                tile_width = min(max(doc.width / max(col_count, 1) - 30, 120), 220)
                tiles: List[Any] = []
                for idx, (title_txt, subtitle_txt, _) in enumerate(normalized):
                    border_color = _mix_colors(accent_color, colors.white, 0.7)
                    tile_background = colors.white
                    rows: List[List[Any]] = []
                    rows.append([Paragraph(title_txt, styles['CardValue'])])
                    rows.append([Paragraph(subtitle_txt, styles['Overline'])])
                    tile = Table(
                        rows,
                    ...
                    tile.setStyle(
                        TableStyle(
                            [
                                ("BACKGROUND", (0, 0), (-1, -1), tile_background),
                    ...
                    tile.hAlign = "CENTER"
                    tiles.append(tile)
                grid = Table([tiles], colWidths=[tile_width] * col_count)
                grid.setStyle(
                    TableStyle(
                        [
                            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ...
            def _build_diagnostic_comparison(rows: List[Tuple[str, str, str]]) -> Optional[Table]:
                if not rows:
                    return None
                header = [
                    Paragraph("<b>Canal / criterio</b>", styles['Normal']),
                    ...
                    TableStyle(
                        [
                            ...
                            ("ROWBACKGROUNDS", (0, 1), (-1, -1), [
                                colors.HexColor("#fefefe"),
                                _mix_colors(accent_color, colors.white, 0.9),
                            ]),
                            ("WORDWRAP", (0, 0), (-1, -1), True),
                            ("GRID", (0, 0), (-1, -1), 0.25, _mix_colors(accent_color, colors.white, 0.6)),
                        ]
                    )
                )
                return table
            ...
            system_metric_tiles = [
                ...
                ("ISO 20816-3", "Referencia normativa"),
            ]
            metric_tiles = _build_metric_tiles(system_metric_tiles)
            ...
            overview_axis_text = self._compose_axis_narrative(
                axis_summaries_pdf or [],
                global_label=severity_global_label,
                global_rms=global_rms_mm_pdf,
            )
            overview_body: List[Any] = [
                ...
                Paragraph(overview_axis_text, styles['Normal']),
            ]
            if severity_table is not None:
                overview_body.append(Spacer(1, 8))
                overview_body.append(Paragraph(f"Semáforo ISO – {iso_scope_label_pdf}", styles['Muted']))
                overview_body.append(Spacer(1, 4))
                overview_body.append(severity_table)
                if iso_scope_note_pdf:
                    overview_body.append(Spacer(1, 4))
                    overview_body.append(Paragraph(iso_scope_note_pdf, styles['Muted']))
            ...
            axis_narrative_pdf = self._compose_axis_narrative(
                axis_summaries_pdf or [],
                global_label=severity_global_label,
                global_rms=global_rms_mm_pdf,
            )
            findings_paragraph = self._compose_findings_paragraph(exec_findings)
            insights_body: List[Any] = [Paragraph("Hallazgos clave", styles['CardTitle'])]
            insights_body.append(Paragraph(axis_narrative_pdf, styles['Normal']))
            insights_body.append(Paragraph(findings_paragraph, styles['Normal']))
            ...
            if axis_table_rows:
                header_bg = _mix_colors(accent_color, colors.white, 0.85)
                axis_table = Table(axis_table_rows)
                axis_table.hAlign = "CENTER"
                axis_table.setStyle(
                    TableStyle(
                        [
                            ("BACKGROUND", (0, 0), (-1, 0), header_bg),
            ...
            if exp_lines_pdf2:
                exp_text_pdf = self._compose_findings_paragraph(exp_lines_pdf2)
                exp_card = _pdf_card(
                    [
                        Paragraph("Explicación y recomendaciones", styles['CardTitle']),
                        Paragraph(exp_text_pdf, styles['Normal']),
                    ],
                    accent_color,
                    background="#fdfdff",
                )
            ...
            axis_rows_for_comparison = self._sort_axis_entries(
                entry for entry in (axis_summaries_pdf or []) if not entry.get("is_global")
            )
            axis_rows_for_comparison = axis_rows_for_comparison[:3]
            comparison_rows: List[Tuple[str, str, str]] = []
            if axis_rows_for_comparison:
                for entry in axis_rows_for_comparison:
                    name = str(entry.get("name", "Canal"))
                    try:
                        rms_local = float(entry.get("rms_mm_s", 0.0))
                        rms_txt = f"{rms_local:.3f} mm/s"
                    except Exception:
                        rms_txt = "N/D"
                    iso_local = entry.get("iso_label", "N/D")
                    classification_bits = [f"ISO: {iso_local}"]
                    comments_bits = [f"RMS {rms_txt}"]
                    ml_summary_entry = entry.get("ml_summary") or {}
                    ml_status_entry = str(ml_summary_entry.get("status") or "").lower()
                    if ml_status_entry == "ok":
                        ml_axis_label = (
                            ml_summary_entry.get("final_label")
                            or ml_summary_entry.get("ml_label")
                            or "OK"
                        )
                        classification_bits.append(f"ML: {ml_axis_label}")
                        try:
                            frac_high_axis = float(ml_summary_entry.get("frac_high_pct", 0.0))
                        except Exception:
                            frac_high_axis = 0.0
                        try:
                            r2x_axis = float(ml_summary_entry.get("r2x", 0.0))
                        except Exception:
                            r2x_axis = 0.0
                        comments_bits.append(f"Energia >120 Hz {frac_high_axis:.1f}%")
                        comments_bits.append(f"Relacion 2X {r2x_axis:.2f}X")
                        if ml_summary_entry.get("conflict"):
                            comments_bits.append("Conflicto ISO/ML")
                    elif ml_status_entry:
                        classification_bits.append("ML: No disponible")
                        comments_bits.append(ml_summary_entry.get("message") or "Modelo ML no disponible.")
                    else:
                        classification_bits.append("ML: Sin resultado")
                        comments_bits.append("Modelo ML no evaluado para este eje.")
                    comments_bits.append(f"Comparacion directa del canal {entry.get('name', 'Eje')} con los umbrales ISO.")
                    comment_text = ". ".join(bit.strip() for bit in comments_bits if bit).strip()
                    if comment_text and not comment_text.endswith("."):
                        comment_text += "."
                    comparison_rows.append(
                        (
                            name,
                            " | ".join(classification_bits),
                            comment_text,
                        )
                    )
            if not comparison_rows:
                comparison_rows = [
                    (
                        ...
                        f"Evaluacion del {primary_name_pdf} conforme a los umbrales ISO 10816/20816.",
                    ),
                ]
            if global_entry_pdf:
                comparison_rows.append(
                    (
                        ...
                        "RMS global calculado como media cuadratica de los ejes disponibles.",
                    )
                )
            comparison_rows.append(
                (
            ...
            axis_detail_note = self._compose_axis_narrative(
                axis_summaries_pdf or [],
                global_label=severity_global_label,
                global_rms=global_rms_mm_pdf,
            )
            if ml_status_value == 'ok':
                ...
                axis_clause = axis_detail_note if axis_detail_note.endswith('.') else f"{axis_detail_note}."
                discrepancy_note = (
                    f"{axis_clause} El modelo ML identifica la condicion global como <b>{ml_base}</b> apoyandose en la energia de alta frecuencia ({frac_high_pct:.1f}%) y en una relacion 2X de {ml_r2x:.2f}. "
                    f"El esquema hibrido ISO+ML adopta finalmente la condicion <b>{ml_label_display}</b>."
                )
                if conflict_pdf:
                    discrepancy_note += ' <b>Existe conflicto elevado entre ISO y ML; valide la condicion con inspeccion adicional.</b>'
            else:
                discrepancy_note = (
                    f"{axis_detail_note} El modelo de Machine Learning no emitio una clasificacion confiable para esta "
                    'medicion, por lo que se prioriza el criterio de la norma ISO dada la energia vibratoria registrada.'
                )
            ...
            diagnostic_contents: List[Any] = [
                ...
                Paragraph(axis_detail_note, styles['Normal']),
            ]
                ...
                ft.Row(
                    ...
                    controls=[
                        ...
                        ft.OutlinedButton(
                            ...
                            on_click=lambda _: self.page.launch_url("https://drive.google.com/file/d/13i3CQJPjgWHv8a8aNBa4RGHBZei63n8n/view?usp=sharing")
...
,
    ...
    def _build_ml_summary_card(
        self,
        ml_payload: Optional[Dict[str, Any]],
        axis_summaries: Optional[List[Dict[str, Any]]] = None,
    ) -> Optional[ft.Control]:
        """Renderiza un apartado visual con el resultado del modelo de ML y cada eje en la UI."""

        if not ml_payload and not axis_summaries:
            return None
        ...
        def _build_axis_overview_section() -> Optional[ft.Control]:
            axis_entries = self._sort_axis_entries(
                entry for entry in (axis_summaries or []) if isinstance(entry, dict) and not entry.get('is_global')
            )
            axis_entries = axis_entries[:3]
            if not axis_entries:
                return None
            axis_rows: List[ft.Control] = []
            for entry in axis_entries:
                label = str(entry.get('name', 'Eje'))
                if entry.get('is_primary') and 'analizado' not in label.lower():
                    label = f"{label} (analizado)"
                try:
                    rms_val = float(entry.get('rms_mm_s', 0.0))
                    rms_txt = f"{rms_val:.3f} mm/s"
                except Exception:
                    rms_txt = 'N/D'
                iso_label = entry.get('iso_label', 'N/D')
                color_hex = entry.get('color') or accent
                ml_summary_entry = entry.get('ml_summary') or {}
                ml_status_entry = str(ml_summary_entry.get('status') or '').lower()
                classification_parts = [f"ISO: {iso_label}"]
                info_bits = [f"RMS {rms_txt}"]
                if ml_status_entry == 'ok':
                    ml_text = (
                        ml_summary_entry.get('final_label')
                        or ml_summary_entry.get('ml_label')
                    ...
                    classification_parts.append(f"ML: {ml_text}")
                    try:
                        frac_high_pct = float(ml_summary_entry.get('frac_high_pct', 0.0))
                    except Exception:
                        frac_high_pct = 0.0
                    try:
                        r2x_val = float(ml_summary_entry.get('r2x', 0.0))
                    except Exception:
                        r2x_val = 0.0
                    info_bits.append(f"Energia alta {frac_high_pct:.1f}%")
                    info_bits.append(f"Relacion 2X {r2x_val:.2f}X")
                    if ml_summary_entry.get('conflict'):
                        info_bits.append('Conflicto ISO/ML')
                elif ml_status_entry:
                    ml_text = ml_summary_entry.get('message') or 'ML no disponible'
                    classification_parts.append(f"ML: {ml_text}")
                else:
                    classification_parts.append('ML: Sin resultado')
                    info_bits.append('Ejecuta el modelo para este eje')
                axis_rows.append(
                    ft.Container(
                        border=ft.border.all(1, Colors.with_opacity(0.2, color_hex)),
                        border_radius=8,
            ...
            return ft.Column(
                [
                    ft.Text('Diagnostico por eje', weight='bold', size=13),
                    ft.Column(axis_rows, spacing=6, tight=True),
                ],
                spacing=6,
            )
        ...
        if status == 'ok':
            ...
            if pattern_section_controls:
                body_controls.append(
                    ft.Container(
                        ft.Column(pattern_section_controls, spacing=6),
                        padding=ft.padding.only(top=6),
                    )
                )

            axis_section = _build_axis_overview_section()
            if axis_section is not None:
                body_controls.append(axis_section)
        ...
        message = ml_result.get('message') or (ml_payload or {}).get('message') or 'Modelo no disponible.'
        ...
        fallback_section = _build_axis_overview_section()
        ...
        if fallback_section is not None:
            column_children.append(fallback_section)
        return ft.Container(
            content=ft.Column(column_children, spacing=8),
            border_radius=12,
            padding=ft.padding.all(16),
    ...
    def _build_analysis_view(self):
        ...
        self.time_unit_dd = ft.Dropdown(
            ...
            value=self.time_display_unit,
            width=220,
            on_change=self._on_time_unit_change,
        )
    ...
    def _generate_trend_figure(self, force_light: bool = False) -> plt.Figure:
        ...
        dark_theme = self.is_dark_mode and not force_light
        face_color = "#0f141b" if dark_theme else "white"
        axis_color = "white" if dark_theme else "black"
    ...
    def _select_main_findings(self, findings: List[str], max_items: int = 2) -> List[str]:
        ...
        try:
            ...
            items = findings[:]
        order = ["Eléctrico", "Eléctrico", "Eléctrico", "Desalineación", "Desbalanceo", "Rodamientos", "Engranes", "Resonancia estructural"]
        selected: List[str] = []
        for key in order:
            for f in items:
                try:
    ...
    def _create_plot(self):
        ...
        try:
            ...
            axis_overview_text = self._compose_axis_narrative(
                axis_summaries or [],
                global_label=global_label,
                global_rms=global_rms_mm,
            )
            if not axis_overview_text:
                axis_overview_text = "No se dispone de un resumen por ejes para esta medicion."
            ...
            if primary_entry.get("ml_bundle"):
                try:
                    ...
                    pass
            if axis_overview_text:
                exp_lines.append(axis_overview_text)
            ...
            if _has("Rodamientos"):
                ...
                if ref_line:
                    exp_lines.append(ref_line)
            if _has("Elctrico") or _has("Eléctrico") or _has("Eléctrico"):
                ...
                for key in ("Elctrico", "Eléctrico", "Eléctrico"):
            ...
            axis_count = self._count_active_axes(axis_summaries)
            iso_scope_label = self._iso_scope_label(axis_count, scope="global")
            iso_scope_note = self._iso_scope_note(axis_count, primary_entry, primary_rms_mm)
            ...
            axis_narrative_ui = axis_overview_text
            exec_findings_text = self._compose_findings_paragraph(exec_findings)
            resumen_exec = ft.Container(
                content=ft.Column(
                    [
                        ...
                        self._build_severity_traffic_light(
                            global_rms_mm,
                            iso_label=global_label,
                            scope_text=iso_scope_label,
                        ),
                        *(
                            [
                                ft.Text(
            ...
            findings_text = self._compose_findings_paragraph(findings)
            ...
            ml_card = self._build_ml_summary_card(res.get("ml"), axis_summaries)
            column_controls: List[ft.Control] = [resumen_exec]
            if ml_card is not None:
                column_controls.append(ml_card)

            exp_text_ui = self._compose_findings_paragraph(exp_lines)
            ...
            conclusion_parts_ui.append(axis_overview_text)
            conclusion_parts_ui.append(
    ...
    def _change_accent_color(self, e):
        ...
        try:
            ...
            if value is not None:
                ...
                self._refresh_loading_overlay_styles()
    ...
    def _build_loading_overlay(
        ...
        """Crea un contenedor modal reutilizable para estados de carga."""
        icon_control = ft.Icon(icon, size=28, color=self._accent_ui())
        ...
        progress_bar = ft.ProgressBar(width=240, height=6, color=self._accent_ui())
        card_container = ft.Container(
            ...
            shadow=ft.BoxShadow(
                spread_radius=0,
                blur_radius=28,
                color=Colors.with_opacity(0.45, "black"),
        ...
        if not hasattr(self, "_loading_overlays"):
            self._loading_overlays = {}
        self._loading_overlays[key] = {
            "overlay": overlay,
            "card": card_container,
            "icon_wrap": icon_wrap,
            "icon": icon_control,
        ...
        self._refresh_loading_overlay_styles(key)
        return overlay


    def _get_loading_overlay_tokens(self) -> Dict[str, Any]:
        """Colores base para los modales de carga según el tema."""
        dark = getattr(self, "is_dark_mode", True)
        return {
            "scrim": Colors.with_opacity(0.55, "black"),
            "card_bg": "#1c2336" if dark else "#ffffff",
            "text": "#f3f6ff" if dark else "#1a2130",
            "hint": "#c3cada" if dark else "#545e73",
            "icon_bg": Colors.with_opacity(0.08, "white" if dark else "black"),
            "icon_border": Colors.with_opacity(0.2, "white" if dark else "black"),
            "track": Colors.with_opacity(0.18, "white" if dark else "black"),
        }


    def _refresh_loading_overlay_styles(self, key: Optional[str] = None) -> None:
        """Actualiza colores y contraste de los overlays activos."""
        overlays: Dict[str, Dict[str, Any]] = getattr(self, "_loading_overlays", {})
        if not overlays:
            return
        tokens = self._get_loading_overlay_tokens()
        accent = self._accent_ui()
        keys = [key] if key else list(overlays.keys())
        for overlay_key in keys:
            controls = overlays.get(overlay_key)
            if not controls:
                continue
            overlay = controls["overlay"]
            overlay.bgcolor = tokens["scrim"]
            card = controls["card"]
            card.bgcolor = tokens["card_bg"]
            icon_wrap = controls["icon_wrap"]
            icon_wrap.bgcolor = tokens["icon_bg"]
            icon_wrap.border = ft.border.all(1, tokens["icon_border"])
            icon = controls["icon"]
            icon.color = accent
            progress = controls["progress"]
            progress.color = accent
            progress.bgcolor = tokens["track"]
            message = controls["message"]
            message.color = tokens["text"]
            message.text_align = ft.TextAlign.CENTER
            hint = controls["hint"]
            hint.color = tokens["hint"]
            hint.text_align = ft.TextAlign.CENTER
            for ctrl in (overlay, card, icon_wrap, icon, progress, message, hint):
                try:
                    if ctrl.page:
                        ctrl.update()
                except Exception:
    ...
    def _get_time_display_unit(self) -> str:
        valid_units = {"vel_mm", "acc", "acc_g"}
        unit_attr = getattr(self, "time_display_unit", None)
        if unit_attr in valid_units:
            return unit_attr
        try:
            ctrl = getattr(self, "time_unit_dd", None)
            if ctrl and ctrl.value in valid_units:
                return ctrl.value
        except Exception:
            pass
        return "vel_mm"

    def _on_time_unit_change(self, e=None):
        valid_units = {"vel_mm", "acc", "acc_g"}
        unit = getattr(getattr(e, "control", None), "value", None) if e else None
        if unit not in valid_units:
            unit = getattr(self, "time_display_unit", "vel_mm")
        if unit not in valid_units:
            unit = "vel_mm"
        self.time_display_unit = unit
        ctrl = getattr(self, "time_unit_dd", None)
        if ctrl:
            ctrl.value = unit
            if ctrl.page:
                ctrl.update()
        try:
            self.page.client_storage.set("time_display_unit", unit)
        except Exception:
            pass
        self._update_analysis()
